...
Gateways must present a token linked to policies that grant the appropriate set of permissions. For a terminating gateway to be discoverable and to forward traffic out of the mesh, the token must be linked to policies that grant the following permissions:
...
The following example policy allows a terminating gateway to register as a service named `terminating-gateway`. For this example, the terminating gateway forwards traffic for two services named `external-service-1` and `external-service-2`.
...
#### Consul Enterprise using `default` partition and `default` namespace

You can specify an admin partition and namespace when using Consul Enterprise. The following example policy allows a terminating gateway to register as a service named `terminating-gateway` in the `default` partition and `default` namespace.
...
#### Consul Enterprise using a non-`default` partition and non-`default` namespace
...
Run the `consul acl policy create` command and specify the policy rules to create a policy. The following example registers a custom policy defined in `tgw-register.hcl`:
...
Send a PUT request to the `/acl/policy` endpoint and specify the policy rules in the request body to create a policy. The following example registers the custom policy defined in `tgw-register.hcl`. You must embed policy rules in the `Rules` field of the request body.
...
You can specify an admin partition when creating tokens in Consul Enterprise. The token is only valid in the specified admin partition. You must create the token in the same partition where the terminating gateway is registered. The following example creates the token in the partition `ptn1` and namespace `default`.
