...
func (suite *nodeHealthControllerTestSuite) SetupTest() {
	...
	suite.runtime = controller.Runtime{Client: suite.resourceClient}

	// The rest of the setup will be to prime the resource service with some data
	suite.nodeNoHealth = ctltest.Resource(types.NodeType, "test-node-no-health").
		WithData(suite.T(), nodeData).
		Write(suite.T(), suite.resourceClient).Id

	suite.nodePassing = ctltest.Resource(types.NodeType, "test-node-passing").
		WithData(suite.T(), nodeData).
		Write(suite.T(), suite.resourceClient).Id

	suite.nodeWarning = ctltest.Resource(types.NodeType, "test-node-warning").
		WithData(suite.T(), nodeData).
		Write(suite.T(), suite.resourceClient).Id

	suite.nodeCritical = ctltest.Resource(types.NodeType, "test-node-critical").
		WithData(suite.T(), nodeData).
		Write(suite.T(), suite.resourceClient).Id

	suite.nodeMaintenance = ctltest.Resource(types.NodeType, "test-node-maintenance").
		WithData(suite.T(), nodeData).
		Write(suite.T(), suite.resourceClient).Id
	...
	for _, node := range []*pbresource.ID{suite.nodePassing, suite.nodeWarning, suite.nodeCritical, suite.nodeMaintenance} {
		for idx, health := range precedenceHealth {
			if nodeHealthDesiredStatus[node.Name] >= health {
				ctltest.Resource(types.HealthStatusType, fmt.Sprintf("test-check-%s-%d", node.Name, idx)).
	...
	ctltest.Resource(types.DNSPolicyType, "test-policy").
...
func (suite *nodeHealthControllerTestSuite) TestReconcilePropagateReadError() {
	...
	// from the initial resource read get propagated. This cases is very unrealsitic
...
func (suite *nodeHealthControllerTestSuite) TestController() {
	...
	ctltest.Resource(types.NodeType, suite.nodePassing.Name).
	...
	ctltest.Resource(types.HealthStatusType, "failure").
