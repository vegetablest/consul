...
func TestValidateServiceEndpoints_EndpointInvalid(t *testing.T) {
	...
	cases := map[string]testCase{
		"invalid-target": {
			...
			validateErr: func(t *testing.T, err error) {
				require.True(t, errors.Is(err, ErrInvalidReferenceType{AllowedType: WorkloadType}))
			},
		},
		"invalid-address": {
			modify: func(endpoint *pbcatalog.Endpoint) {
			...
			validateErr: func(t *testing.T, err error) {
				require.True(t, errors.Is(err, ErrInvalidPortReference{Name: "bar"}))
			},
		},
		"no-ports": {
			modify: func(endpoint *pbcatalog.Endpoint) {
		...
		"port-0": {
			...
			validateErr: func(t *testing.T, err error) {
				require.True(t, errors.Is(err, errInvalidPhysicalPort))
			},
		},
		"port-out-of-range": {
			modify: func(endpoint *pbcatalog.Endpoint) {
