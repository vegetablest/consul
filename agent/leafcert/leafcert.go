...
const (
	...
	// This defaults to not being unlimited
...
type Config struct {
	...
	// the cache after the last get operation. If a certs isn't accessed within
...
func (m *Manager) internalGet(ctx context.Context, req *ConnectCALeafRequest) (*structs.IssuedCert, cache.ResultMeta, error) {
	...
	timeoutCh := time.After(req.MaxQueryTime)
...
func (m *Manager) runExpiryLoop() {
	for {
		...
		case <-timer.Wait():
			...
			entry := timer.Entry
			if cert, ok := m.certs[entry.Key()]; ok {
				cert.Stop()
			}
